...
- See https://example.com/security/encryption.md for details
...
class EncryptionService:
    ...
    def decrypt_secret_or_plaintext(self, bundle_json: str) -> Optional[str]:
        ...
        if not is_encrypted:
            ...
            # we return it as-is (knowing it was supposed to be encrypted)
            if bundle_json.startswith(self.FORMAT_MARKER):
                # Has v2: prefix but failed validation - corrupted encrypted data
                # Return as-is so caller can debug
                return bundle_json

            # For data without encryption markers that looks like garbage/failed encryption,
            # return None. Otherwise return plaintext.
            if self._looks_like_failed_encryption(bundle_json):
                logger.error("Input looks like failed encryption attempt: %s", bundle_json[:50])
                return None

            # Regular plaintext or non-encrypted data
            return bundle_json
    ...
    def _looks_like_failed_encryption(self, text: str) -> bool:
        """Heuristic: does this look like a failed/corrupted encryption attempt?

        Returns True (treat as None) if:
        - Text decodable as base64 (suspicious for regular plaintext)
        - Text is 20-200 chars (length of a typical encrypted token)
        - Doesn't contain spaces or special chars (unlikely plaintext)

        Returns False (treat as plaintext) if:
        - Text is too short or too long for encryption
        - Text contains spaces or punctuation (likely real plaintext)
        - Text contains obviously non-base64 chars

        Args:
            text: Text to check

        Returns:
            bool: True if looks like failed encryption attempt, False if looks like plaintext
        """
        # Very short or very long strings are probably plaintext
        if len(text) < 20 or len(text) > 500:
            return False

        # If it contains spaces, newlines, or common punctuation, treat as plaintext
        ...
        try:
            ...
            # It decoded as base64, but not recognized as valid encryption
            # This is suspicious - could be corrupted
            return True
        except Exception:
            # Doesn't decode as base64, so treat as plaintext
            return False
